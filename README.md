...
| `datasheets.md` | **Generated.** A primer for a returning player, stat sheets for every datasheet in the collection (characteristics, weapons, ability gists, a strategy per unit), and a glossary |
| `datasheets-deck.html` | The same content as a slide deck (phone-usable, no external resources); every glossary term shows its definition on hover or tap |
| `data/datasheets.json` / `data/glossary.json` | Datasheet facts extracted from BSData/wh40k-11e (numbers and names only) plus hand-written gists and strategy; the glossary behind the primer and the tooltips |
